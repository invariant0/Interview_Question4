...
def evaluate_model_on_2d_grid(
    ...
    Args:
        model: Value function neural network.
        normalizer: State space normalizer.
        k_grid: Capital grid values.
        b_grid: Debt grid values.
        z_value: Fixed productivity value.
        
    Returns:
        2D array of predictions with shape (n_k, n_b).
    """
...
def compute_default_probability_vfi(
    ...
    Default probability is 1 where value <= threshold, 0 otherwise.
    
    Args:
        vfi_values: VFI value function array.
        default_threshold: Threshold below which state is in default.
        
    Returns:
        Binary array indicating default (1) or non-default (0).
    """
...
def compute_default_probability_dl(
    ...
    Args:
        dl_values: DL value function array.
        default_threshold: Threshold below which state is in default.
...
def plot_3d_value_and_default_comparison(
    ...
    for col_idx, shock in enumerate(shock_levels):
        ...
        legend_elements = [
            ...
            Patch(facecolor='#ff6600', alpha=0.8, edgecolor='darkred', label='DL')
        ]
        ...
        legend_elements_default = [
            ...
            Patch(facecolor=dl_color_solid, alpha=0.8, edgecolor='#cc4400', label='DL')
        ]
    ...
    # Add overall title
    fig.suptitle(
        'VFI vs Deep Learning: Value Function and Default Probability Comparison',
...
class BellmanResidualCalculator:
    ...
    The Bellman residual measures how well the value function satisfies:
        V(k, b, z) = max_{k', b'} { d(k, b, z, k', b') + beta * E[V(k', b', z')] }
    
    where d is the dividend and the expectation is over future productivity shocks.
    """
    ...
    def __init__(
        ...
        """
        Initialize the Bellman residual calculator.
    ...
    def sample_evaluation_states(
        ...
        """
        Sample a fixed batch of states for Bellman residual evaluation.
        
        Uses the StateSampler with full progress (progress=1.0) to sample
        ...
        Returns:
            Tuple of (k, b, z) tensors, each with shape (batch_size, 1).
        """
        if seed is not None:
    ...
    def compute_bellman_residual(
        ...
        """
        Compute the Bellman residual for a batch of states.
        
        The Bellman residual is defined as:
            residual = V(k, b, z) - max_{k', b'} { d + beta * E[V(k', b', z')] }
        
        A well-converged value function should have residuals close to zero.
        
        Args:
            model: Value function neural network.
            k: Current capital values, shape (batch_size, 1).
            b: Current debt values, shape (batch_size, 1).
            z: Current productivity values, shape (batch_size, 1).
            n_candidates: Number of candidate (k', b') pairs to evaluate.
            n_mc_samples: Number of MC samples for continuation value.
            
        Returns:
            Bellman residuals, shape (batch_size,).
        """
...
def compute_bellman_residual_stats(
    ...
    Args:
        model: Value function neural network.
        calculator: Bellman residual calculator.
        k_eval: Evaluation capital values.
        b_eval: Evaluation debt values.
        z_eval: Evaluation productivity values.
...
def main():
    ...
    fig = plot_training_metrics(
        epochs, maes, mapes_non_default, f1_scores, bellman_residuals, output_dir
    )
    plt.close(fig)
