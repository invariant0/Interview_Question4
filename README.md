...
*   **Learning rate scheduler**: This is arguably the most consequential hyperparameter to tune. After experimentation, we found success setting the overall learning rate to 0.001. The policy learning rate should match the value network (a policy scale of 1.0); setting the policy rate too low will cause the model to converge to a local minimum. The contradiction from the acedemic common practice to set policy lr to half of the value net lr is probability because of the pretraining phase practiced in this experiments
