#!/usr/bin/env python3
"""Compare VFI and Deep Learning (projection model) simulation results for the risky debt model.

This script loads a ground-truth VFI solution and DL projection-model
checkpoints at several training epochs, simulates both, and produces
two figures:
...
Usage::
    ...
    python risky_projection_simulation.py [--econ-id 0] [--gpu 0]
"""
...
from src.econ_models.simulator import DLSimulatorRiskyProjection
...
RESULTS_DIR: str = "./results/effectiveness_risky_projection"
...
def load_dl_simulator(econ_params, bonds_config):
    """Create and configure a DL simulator for the risky projection model."""
    dl_config = load_dl_config("./hyperparam/prefixed/dl_params.json", "risky_projection")
    ...
    return DLSimulatorRiskyProjection(dl_config, econ_params)


def run_dl_epoch(
    dl_simulator: DLSimulatorRiskyProjection,
    epoch: int,
    initial_states,
    shock_sequence,
    burn_in: int,
    ...
    dl_simulator.load_solved_dl_solution(
        invest_policy_filepath=os.path.join(
            CHECKPOINT_DIR, f"risky_invest_policy_net_{epoch}.weights.h5"
        ),
        wait_policy_filepath=os.path.join(
            CHECKPOINT_DIR, f"risky_wait_policy_net_{epoch}.weights.h5"
        ),
        branch_selection_filepath=os.path.join(
            CHECKPOINT_DIR, f"risky_branch_selection_net_{epoch}.weights.h5"
        ),
        continuous_net_filepath=os.path.join(
            CHECKPOINT_DIR, f"risky_continuous_net_{epoch}.weights.h5"
        ),
    )
...
def plot_stationary_distributions(
    ...
    fig.suptitle(
        f"Risky Projection Model: Stationary Distribution Comparison  (Econ ID: {econ_id})",
...
def plot_moments_figure(
    ...
    n_rows_vars = (n_vars + 1) // 2
    n_rows = n_rows_vars + 1

    fig, axes = plt.subplots(n_rows, 2, figsize=(24, 7 * n_rows))
    fig.suptitle(
        f"Risky Projection Model: Moments & Frequencies — VFI vs. Deep Learning  (Econ ID: {econ_id})",
        fontsize=18, fontweight="bold", y=0.99,
    )
...
def plot_sample_trajectories(
    ...
    *,
    ...
    """Plot time-series trajectories for a single sampled firm."""
    ...
    fig.suptitle(
        f"Projection Model: Sample Firm Trajectory  (Firm {firm_idx}, Econ ID: {econ_id})",
...
def main() -> None:
    """Run VFI and multi-epoch projection DL simulations, then generate comparison figures."""
    parser = argparse.ArgumentParser(
        description="Risky projection model: DL vs VFI simulation comparison"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gpu", type=int, default=0, choices=range(4),
        help="GPU device ID to use (0-3, default: 0)",
    )
    args = parser.parse_args()

    # --- Restrict to the specified GPU ---
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
    gpus = tf.config.list_physical_devices("GPU")
    if gpus:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)
        print(f"Using GPU {args.gpu} (visible devices: {[g.name for g in gpus]})")
    else:
        print(f"WARNING: --gpu {args.gpu} requested but no GPU visible to TensorFlow")
    ...
    for epoch in DL_EPOCHS:
        print(f"\nRunning DL projection epoch {epoch} ...")
        result = run_dl_epoch(
    ...
    outfile1 = os.path.join(RESULTS_DIR, "risky_projection_simulation_distributions.png")
    ...
    outfile2 = os.path.join(RESULTS_DIR, "risky_projection_moments_combined.png")
    ...
    outfile3 = os.path.join(RESULTS_DIR, "risky_projection_simulation_trajectories.png")
    fig3.savefig(outfile3)
    print(f"Figure 3 saved to {outfile3}")

    print("\nProjection model simulation and plotting completed successfully.")
